// src/assets/design/logo-marks.tsx
...
export const EMERALD = "#0B5D3B";
export const EMERALD_DEEP = "#06281C";
export const GOLD = "#D4AF63";
export const GOLD_SOFT = "#E6C786";
export const IVORY = "#F7F4ED";
export const ON_DARK = "#F2EEE4";

/* ---------- Palm canopy (drawn around apex of the A) ---------- */
function PalmCanopy({ cx, cy, color, accent }: { cx: number; cy: number; color: string; accent: string }) {
  const fronds = [
  ...
  const buildFrond = ({ angle, len, droop }: { angle: number; len: number; droop: number }) => {
  ...
  const leafletsFor = (f: ReturnType<typeof buildFrond>) => {
    ...
    for (let i = 1; i < N; i++) {
      ...
      const nx = -dy / len;
      const ny = dx / len;
      const leafLen = 6.5 * (1 - t * 0.45);
      const sides = Math.abs(f.angle) > 60 ? [1] : [-1, 1];
      sides.forEach((s) => {
        ticks.push({ x1: p.x, y1: p.y, x2: p.x + nx * leafLen * s, y2: p.y + ny * leafLen * s });
      });
    }
    return ticks;
  };
...
export function AldiraGlyph({ primary = EMERALD, accent = GOLD }: { primary?: string; accent?: string }) {
  const apex = { x: 100, y: 76 };
  return (
    <g>
...
/* ---------- Wordmark ---------- */
export function AldiratechWordmark({
  color = EMERALD,
  accentColor = GOLD,
  showTagline = true,
  partner = true,
  ...
  return (
    <g>
      <text
        ...
        fill={color}
      >
        <tspan fontWeight="700">Aldira</tspan>
        <tspan fontWeight="400">tech</tspan>
      </text>
      {showTagline && (
        <text
          x="2"
...
export function Favicon({ size = 64, mode = "solid" }: { size?: number; mode?: "solid" | "transparent" }) {
